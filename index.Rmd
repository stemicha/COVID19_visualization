...
mGT <- generation.time(type = "gamma",val =  c(3,2))
TD <- est.R0.TD(rev(Germany_cases_100_vector), mGT, nsim=10000)## Results will include "most likely R(t)" (ie. the R(t)
R0_Germany <- tibble(dateRep = names(TD$R),R0 = TD$R,lower_CI = TD$conf.int$lower,upper_CI = TD$conf.int$upper)
...
R0_Germany_plot<- ggplot(R0_Germany,aes(as_date(dateRep),R0))+
  ...
  theme_ipsum_rc(base_size = 18)+
  labs(title = "Germany: R0",subtitle = "generation time (gamma distribution with a mean of 3 (SD=2)")+
